...
class DDoSPlaybook:
    ...
    def execute(self, data):
        ...
        FORMAT = "40s 20s I 200s 10s 50s 50s 50s 500s 50s"
        ...
        if isinstance(data["description"], dict):
            data["description"] = json.dumps(data["description"])[:200]  # Convert to string, truncate to 200 bytes

        print("\n🔍 Debug: Final Data Before Packing:")
        for key, value in data.items():
            print(f"{key}: {value} (Type: {type(value)})")

        print(f"✅ DDoS Attack detected, sending message to {self.waf_ip}:{self.waf_port}")
